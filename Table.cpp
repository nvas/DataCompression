...
void Table:: methodStdTable(){
    ...
    for(int row=0;row<=MainVector.size();row++)
    {
        ...
        {
            double call_get_average = getAverage_column(col);
            double call_get_std_dev = getSampleStdDevOfColumn(col);
            standardised_cell = (MainVector[row][col]- call_get_average) / call_get_std_dev;
